  ...
  return (
    <>
      ...
      <div className="cursor-pointer">
        <b className="play">Players List</b>
        <div className="flex flex-row flex-wrap justify-between w-full h-full bg-slate-900 text-slate-50">
          {players &&
            players.map((player, index) => (
              <div
                key={index}
                className="hover:bg-slate-800 justify-center "
                onClick={() => handlePlayer(player)}
              >
                <img
                  className="border-red-50 m-10 "
                  src={`/${player.image}`}
                  alt="Rohit Sharma"
                />
                <div className="justify-end p-auto mx-0 items-center">
                  <div>
                    <b>Name: </b>
                    {player.name}
                  </div>
